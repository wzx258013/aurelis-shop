...
import { cmsConfigured, fallbackSettings, supabase, type CmsProduct, type SiteSettings } from "@/lib/cms";
...
export default function AdminPage() {
  ...
  async function submitAuth(event: FormEvent) {
    ...
    else setMessage(authMode === "login" ? "Signed in." : "Account created. Check your email if confirmation is enabled.");
  }
...
function ProductEditor({ product, onClose, onSaved }: { product: CmsProduct; onClose: () => void; onSaved: (product: CmsProduct) => void }) {
  ...
  async function save(event: FormEvent) { event.preventDefault(); if (!supabase) return; setSaving(true); setError(""); const payload = { ...draft, id: draft.id || undefined, detail_image: undefined }; const { data, error: saveError } = await supabase.from("products").upsert({ id: payload.id, slug: draft.slug, name: draft.name, cn: draft.cn, price: draft.price, tag: draft.tag, category: draft.category, material: draft.material, description: draft.description, image: draft.image, detail_image: draft.detailImage, published: draft.published, sort_order: draft.sort_order }).select().single(); if (saveError) setError(saveError.message); else onSaved(data as CmsProduct); setSaving(false); }
