...
import { fetchPublishedProducts, fallbackCmsProducts } from "@/lib/cms";

export default function CollectionPage() {
  const [products, setProducts] = useState(fallbackCmsProducts);
  useEffect(() => { fetchPublishedProducts().then(setProducts); }, []);
  return (
    <main className="inner-page">
      ...
      <section className="collection-grid">{products.map((product) => <Link href={`/product/?slug=${product.slug}`} className="product-card" key={product.slug}><div className="product-image-wrap"><img src={product.image} alt={product.name} /><span className="product-tag">{product.tag}</span><span className="product-arrow"><ArrowUpRight size={18} /></span></div><div className="product-meta"><div><h3>{product.name}</h3><p>{product.material}</p></div><strong>{product.price}</strong></div></Link>)}</section>
